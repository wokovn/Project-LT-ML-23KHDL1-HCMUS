...
def load_audio(file_path):
    ...
    if x.dim() == 1:
        x = x.unsqueeze(0)  # mono: [T] -> [1, T]
    else:
        x = x.T  # stereo: [T, C] -> [C, T]
    if x.shape[0] > 1:
...
class VitsDataset(TTSDataset):
    ...
    def __getitem__(self, idx):
        ...
        pitch = self.get_f0(idx)["f0"] if self.compute_f0 else None
    ...
    def collate_fn(self, batch):
        ...
        pitch_padded = None
        if self.compute_f0:
            pitch_lens = [len(pitch) for pitch in batch["pitch"]]
            max_pitch_len = max(pitch_lens)
            pitch_padded = torch.zeros(B, 1, max_pitch_len)
            for i, pitch in enumerate(batch["pitch"]):
                pitch = np.asarray(pitch, dtype=np.float32).flatten()
                pitch_padded[i, 0, : pitch.shape[0]] = torch.from_numpy(pitch)
...
class VitsPitchPredictor(nn.Module):
    """FastSpeech2-style pitch predictor used as an auxiliary head for VITS."""

    def __init__(self, channels: int, kernel_size: int = 3, dropout_p: float = 0.1):
        super().__init__()
        padding = (kernel_size - 1) // 2
        self.conv_1 = nn.Conv1d(channels, channels, kernel_size=kernel_size, padding=padding)
        self.norm_1 = nn.LayerNorm(channels)
        self.conv_2 = nn.Conv1d(channels, channels, kernel_size=kernel_size, padding=padding)
        self.norm_2 = nn.LayerNorm(channels)
        self.dropout = nn.Dropout(dropout_p)
        self.proj = nn.Conv1d(channels, 1, kernel_size=1)

    def _forward_block(self, x: torch.Tensor, conv: nn.Module, norm: nn.Module) -> torch.Tensor:
        x = conv(x)
        x = F.relu(x)
        x = x.transpose(1, 2)
        x = norm(x)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self._forward_block(x, self.conv_1, self.norm_1)
        x = self._forward_block(x, self.conv_2, self.norm_2)
        return self.proj(x)
...
class Vits(BaseTTS):
    ...
    def __init__(
        ...
        self.pitch_loss_alpha = getattr(config, "pitch_loss_alpha", 0.0)
        if self.pitch_loss_alpha > 0:
            self.pitch_predictor = VitsPitchPredictor(
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        if self.pitch_loss_alpha > 0:
            pitch_pred = self.pitch_predictor(m_p)  # m_p shape: [B, C, T_mel]
        ...
        if optimizer_idx == 1:
            ...
            with autocast(enabled=False):  # use float32 for the criterion
                loss_dict = criterion[optimizer_idx](
                    ...
                    pitch_pred=self.model_outputs_cache.get("pitch_pred"),
                    pitch_target=pitch,
    ...
    def format_batch_on_device(self, batch):
        ...
        if batch.get("pitch") is not None:
            pitch = batch["pitch"].to(dtype=batch["mel"].dtype, device=batch["mel"].device)
            if pitch.shape[2] > batch["mel"].shape[2]:
                pitch = pitch[:, :, : batch["mel"].shape[2]]
            elif pitch.shape[2] < batch["mel"].shape[2]:
                pitch = F.pad(pitch, (0, batch["mel"].shape[2] - pitch.shape[2]))
            pitch = pitch * sequence_mask(batch["mel_lens"]).unsqueeze(1)
            # Convert voiced F0 to log-scale for numerically stable pitch loss.
            # Unvoiced frames remain 0 so the mask (pitch != 0) in the loss still works.
            voiced_mask = pitch > 0
            pitch = pitch.clone()
            pitch[voiced_mask] = torch.log(pitch[voiced_mask])
            batch["pitch"] = pitch
        return batch
    ...
    def get_sampler(self, config: Coqpit, dataset: TTSDataset, num_gpus=1, is_eval=False):
        ...
        weighted_sampler_attrs = getattr(config, "weighted_sampler_attrs", {}) or {}
        weighted_sampler_multipliers = getattr(config, "weighted_sampler_multipliers", {}) or {}

        if getattr(config, "use_weighted_sampler", False):
            for attr_name, alpha in weighted_sampler_attrs.items():
                print(f" > Using weighted sampler for attribute '{attr_name}' with alpha '{alpha}'")
                multi_dict = weighted_sampler_multipliers.get(attr_name, None)
                print(multi_dict)
    ...
    def get_data_loader(
        ...
        if is_eval and not config.run_eval:
            ...
            dataset = VitsDataset(
                ...
                f0_cache_path=getattr(config, "f0_cache_path", None),
            ...
            if sampler is None:
                ...
                if num_gpus > 1:
                    if isinstance(sampler, DistributedSamplerWrapper):
                        # DistributedSamplerWrapper wraps a BucketBatchSampler
                        # that already yields complete batches (lists of indices).
                        # Must use batch_sampler= so DataLoader doesn't re-batch them.
                        loader = DataLoader(
                            dataset,
                        ...
                        # Plain DistributedSampler yields individual indices.
                        loader = DataLoader(
                            dataset,
                            sampler=sampler,
                            batch_size=config.eval_batch_size if is_eval else config.batch_size,
