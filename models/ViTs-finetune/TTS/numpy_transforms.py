...
def compute_f0(
    ...
    **kwargs,
    ...
    try:
        ...
        # Convert hop_length to frame_period in milliseconds
        frame_period = (hop_length / sample_rate) * 1000.0
        
        # Ensure x is strictly 1-dimensional for pyworld
        x_1d = x.flatten().astype(np.float64)
        ...
        f0 = pw.stonemask(x_1d, _f0, t, sample_rate)
        
        # Match lengths in case pw returns slightly different shape than stft
        pad_len = (len(x) // hop_length) + 1 if center else (len(x) - win_length) // hop_length + 1
        if len(f0) > pad_len:
            f0 = f0[:pad_len]
        elif len(f0) < pad_len:
            f0 = np.pad(f0, (0, pad_len - len(f0)), mode="edge")
            
        # Voiced mask threshold
        f0[f0 < pitch_fmin] = 0.0
