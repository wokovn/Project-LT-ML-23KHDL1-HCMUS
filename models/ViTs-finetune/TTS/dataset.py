...
class F0Dataset:
    ...
    def __init__(
        ...
        self.lazy_f0 = str(os.environ.get("TTS_LAZY_F0", "0")).strip().lower() in {"1", "true", "yes", "on"}
        self.pad_id = 0.0
        self.mean = None
        self.std = None
        if cache_path is not None:
            stats_file = os.path.join(cache_path, "pitch_stats.npy")
            need_precompute = not os.path.exists(stats_file)

            # Only rank 0 (or non-DDP) precomputes; others poll for completion
            is_rank_0 = True
            rank_id = 0
            try:
            ...
            if need_precompute and self.lazy_f0:
                if is_rank_0:
                    os.makedirs(cache_path, exist_ok=True)
                print("[*] Lazy F0 mode enabled. Skipping full F0 precompute.")
            elif need_precompute:
                if is_rank_0:
                    os.makedirs(cache_path, exist_ok=True)
                    self.precompute(precompute_num_workers)
                else:
                    # Poll for stats file — avoids NCCL barrier timeout
                    import time
                    print(f"[Rank {rank_id}] Waiting for Rank 0 to finish F0 precompute...")
                    while not os.path.exists(stats_file):
                        time.sleep(30)
                    # Small extra wait to ensure file is fully written
                    time.sleep(5)
                    print(f"[Rank {rank_id}] F0 cache ready.")

        if normalize_f0:
            stats_file = os.path.join(cache_path, "pitch_stats.npy") if cache_path else None
            if stats_file and os.path.exists(stats_file):
                self.load_stats(cache_path)
            else:
                # In lazy mode we can train with raw F0 values and skip global normalization.
                self.normalize_f0 = False
                print("[!] pitch_stats.npy not found. Continuing with unnormalized F0.")
    ...
    def compute_or_load(self, wav_file, audio_unique_name):
        ...
        if self.cache_path is None:
            pitch = self._compute_and_save_pitch(self.ap, wav_file, None)
