...
class VitsGeneratorLoss(nn.Module):
    def __init__(self, c: Coqpit):
        ...
        self.pitch_loss_alpha = getattr(c, "pitch_loss_alpha", 0.0)
    ...
    @staticmethod
    def pitch_contour_loss(pitch_pred, pitch_target):
        if pitch_pred is None or pitch_target is None:
            return None
        pitch_pred = pitch_pred.float()
        pitch_target = pitch_target.float()
        if pitch_pred.shape[-1] > pitch_target.shape[-1]:
            pitch_pred = pitch_pred[:, :, : pitch_target.shape[-1]]
        elif pitch_pred.shape[-1] < pitch_target.shape[-1]:
            pitch_target = pitch_target[:, :, : pitch_pred.shape[-1]]
        mask = (pitch_target != 0.0).float()
        if torch.sum(mask) == 0:
            return torch.zeros((), device=pitch_pred.device, dtype=pitch_pred.dtype)
        return torch.sum((pitch_pred - pitch_target) ** 2 * mask) / torch.sum(mask)
    ...
    def forward(
        ...
        if self.pitch_loss_alpha > 0 and pitch_pred is not None and pitch_target is not None:
            loss_pitch = self.pitch_contour_loss(pitch_pred, pitch_target) * self.pitch_loss_alpha
            loss = loss + loss_pitch
